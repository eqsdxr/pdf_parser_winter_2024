...
class Parser:
    ...
    def fetch_denied_table(
        ...
        denied = []
        if (
            tab[0][1] == "Наименование поставщика"
            or tab[0][1] == "Өнім берушінің атауы"
        ):
            tab = tab[1:]
        for row in tab:
            denied.append(
    ...
    def fetch_results_table(
        ...
        for row in tab:
            if row[1] == "Наименование поставщика" or row[1] == "Өнім берушінің атауы":
                continue
    ...
    def proceed_pdf(self, pdf: str | bytes) -> list[data.ThreeTablesLDR]:
        if isinstance(pdf, str) and "http" not in pdf:
            ...
            raise TypeError("Provide pdf in bytes or provide local path to pdf.")
